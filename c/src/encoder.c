...
int rtimg_encode_lossless_tile(const uint8_t *pixels, size_t pixel_len, uint16_t tile_width, uint16_t tile_height, uint8_t channels, uint8_t predictor_id, uint8_t *out_residuals, size_t out_len) {
    size_t expected = (size_t)tile_width * (size_t)tile_height * (size_t)channels;
    size_t stride = (size_t)tile_width * (size_t)channels;
    size_t y, x;
    uint8_t c;

    if (!pixels || !out_residuals) return RTIMG_ERR_FORMAT;
    if (pixel_len != expected || out_len != expected) return RTIMG_ERR_FORMAT;
    ...
    for (y = 0; y < tile_height; ++y) {
        ...
        for (x = 0; x < tile_width; ++x) {
            for (c = 0; c < channels; ++c) {
                ...
                uint8_t left = (x > 0) ? pixels[idx - channels] : 0;
                uint8_t up = (y > 0) ? pixels[prev_row_off + (x * channels) + c] : 0;
                uint8_t up_left = (x > 0 && y > 0) ? pixels[prev_row_off + ((x - 1) * channels) + c] : 0;
                uint8_t pred = rtimg_predict(predictor_id, left, up, up_left);
                out_residuals[idx] = (uint8_t)((pixels[idx] - pred) & 0xFFu);
            }
        }
    }
